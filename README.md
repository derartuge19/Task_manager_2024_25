...
Key Features
For Users:
User Registration & Login: Users can create accounts to securely log in and manage their tasks.
Task Management: Users can create, update, and delete tasks, assigning them deadlines, priorities, and descriptions.
Task Filtering & Sorting: Tasks can be filtered by status (e.g., pending, completed) and sorted by date or priority to enhance productivity.
Task Progress Tracking: Users can mark tasks as complete and monitor their progress over time.
For Admins (optional role):

User Management: Admins can view and manage user accounts to ensure smooth operation.
Task Overview: Admins have access to an overview of all tasks created by users, with the ability to generate reports for analysis.
Technology Stack
Backend:

NestJS: A powerful, extensible Node.js framework used to build efficient and scalable server-side applications.
TypeScript: A statically typed superset of JavaScript, ensuring enhanced development experiences and safety.
Frontend:
